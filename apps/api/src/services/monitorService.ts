...
import * as monitorRepo from "../repositories/monitorRepository.js";
...
export class MonitorService {
  ...
  async getDashboardState(): Promise<DashboardState> {
    const [recentAlerts, lastSnapshots] = await Promise.all([
      monitorRepo.readRecentAlerts(50),
      Promise.all(this.tokens.map((token) => monitorRepo.readHolderSnapshot(token.mint)))
    ]);

    return {
      tokens: this.tokens,
      recentAlerts: recentAlerts.reverse(),
      recentControlRows: [], // Removed as we don't display all raw rows usually
  ...
  async runSnapshots(): Promise<void> {
    await mapWithConcurrency(this.tokens, this.config.rpcConcurrency, async (token) => {
      ...
      await monitorRepo.saveHolderSnapshot(snapshot);
    });
  }

  async runCorrelationAndAlerts(): Promise<{ rows: number; alerts: number }> {
    const snapshots = (
      await Promise.all(this.tokens.map((token) => monitorRepo.readHolderSnapshot(token.mint)))
    ...
    await monitorRepo.appendControlRows(rows);

    const alertState = await monitorRepo.loadAlertState();
    ...
    // saveAlertState removed as we use DB alerts table
    await monitorRepo.appendAlerts(events);
